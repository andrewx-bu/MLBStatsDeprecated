...
extension FielderStatsView {
    @Observable class ViewModel {
        ...
        var selectedSeasonRange: SeasonRange = .SZN {
            didSet {
                Task {
                    await fetchFielders(startSeason: selectedSeasonRange.startSeason)
                }
            }
        }
        /*
         ...
         */
        func fetchFielders(min inn: Int? = nil, currentSeason: Int = Int(Calendar.current.component(.year, from: Date())), startSeason: Int? = nil, for teamID: Int? = nil) async {
            let inn = inn ?? (startSeason == nil ? 50 : 500)
